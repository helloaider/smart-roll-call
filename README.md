...
- **学生介绍** — 点名后在姓名旁弹出介绍卡片，支持文字介绍和学生照片，20 秒后自动消失；有照片时可点击放大查看
- **学生备注** — 可为每位学生添加仅老师可见的备注，座位左上角显示红点提示，备注内容在点名统计和导出 Excel 中同步展示
...
### v1.4
- 学生介绍支持照片显示：将照片放入 `student_photos/` 目录，文件名与学生姓名一致即可自动匹配，支持 jpg/png/webp/gif 等格式
...
- 规范项目目录结构：新增 `student_photos/`、`assets/`、`examples/` 目录，各类文件职责更清晰
...
- 修复从 Excel 导入座位表时 Office 主题色无法识别的问题
...
- 支持从 Excel / HTML 导入座位配置，支持导出 XLSX
- 支持从点名器菜单直接跳转编辑并应用配置
- 修复过道合并、列宽等显示问题
