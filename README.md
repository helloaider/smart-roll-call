...
- **点名统计** — 支持按时间范围筛选，查看每位学生被点次数、总次数、人均/最大/最小被点次数，支持排序，含备注列
- **统计导出** — 在统计页面可导出当前筛选范围内的数据为 Excel，含点名统计和点名记录两个 Sheet，标题行自动合并居中
...
- **点名记录导入/导出** — 支持将点名数据导出为 Excel 文件存档，可从导出文件恢复，方便跨设备迁移；导入时自动识别并提示不在当前座位表中的姓名
- **一键清空记录** — 清除本轮所有点名记录
...
- 新增统计页面导出：可将当前筛选范围内的统计数据导出为 Excel，含「点名统计」和「点名记录」两个 Sheet，标题行自动合并居中加粗
- 导出 Excel 统一优化：设置菜单的「导出点名记录」同步优化，标题行、时间行自动合并居中，新增导出时间信息
- 导入点名记录增强：导入时自动识别姓名不在当前座位表中的记录，在确认弹窗和结果弹窗中分别提示忽略的姓名和条数
- 修复编辑座位信息时小组颜色未正确传递的问题（rgba 字符串与颜色 key 的转换）
- 修复从 Excel 导入座位表时，使用 Office 主题色（theme 属性）的单元格背景色无法识别的问题
- 点名统计弹窗「记录明细」Tab 重命名为「点名记录」
- 小组预设颜色优化：将薄荷绿替换为青绿（#2DD4BF），靛蓝替换为靛紫（#6366F1），提升相邻小组的颜色区分度
...
- 统计数据仅统计当前座位表中存在的学生，不在座位表中的历史记录不计入统计
